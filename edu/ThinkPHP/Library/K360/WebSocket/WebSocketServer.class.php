...
abstract class WebSocketServer {
  ...
  abstract protected function process(WebSocketUser $user,$message, $socket); // Called immediately when the data is recieved. 
  abstract protected function connected($user);        // Called after the handshake response is sent to the client.
  abstract protected function closed($user);           // Called after the connection is closed.
  abstract protected function readmessage($message, $socket);   // Called when read a message
...
//            $this->stderr("Client disconnected. TCP connection lost: " . $socket);
          	...
          	if($this->readmessage($buffer, $socket) === false){
          		continue;
          	}
            ...
            if (!$user->handshake) {
              ...
              $this->split_packet($numBytes,$buffer, $user, $socket);
            }
          }
        }
      }
...
//        $this->stdout("Client disconnected. ".$disconnectedUser->socket);
  ...
  protected function split_packet($length,$packet, $user, $socket) {
      ...
      if (($message = $this->deframe($frame, $user,$headers)) !== FALSE) {
        if ($user->hasSentClose) {
          ...
          if ((preg_match('//u', $message)) || ($headers['opcode']==2)) {
            ...
            $this->process($user, $message, $socket);
          } else {
