		...
		*/
		obj.init = function () {
			...
			window.addEventListener("resize", function () {
				//重设大小
				var xScrolls = document.querySelectorAll("[kscrollx]");
				var yScrolls = document.querySelectorAll("[kscrolly]");
				for (var i = 0; i < xScrolls.length; i++) {
					var aScroll = xScrolls.item(i);
					var dom = aScroll.parentElement;
					var bar = aScroll.children.item(0);
					setScroll("x", dom, bar);
				}
				for (var i = 0; i < yScrolls.length; i++) {
					var aScroll = yScrolls.item(i);
					var dom = aScroll.parentElement;
					var bar = aScroll.children.item(0);
					setScroll("y", dom, bar);
				}
